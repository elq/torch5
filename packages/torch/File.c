...
{ \
  static TH##CTYPE##Storage torch_File_##TYPE##Storage; \
  long ret; \
\
  ...
  torch_File_##TYPE##Storage.data = data; \
  torch_File_##TYPE##Storage.size = n; \
  torch_File_##TYPE##Storage.refcount = -1; /* makes sure nobody can free it */ \
  luaT_pushudata(L, &torch_File_##TYPE##Storage, torch_##CTYPE##Storage_id); \
  lua_call(L, 2, 1); \
