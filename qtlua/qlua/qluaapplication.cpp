  ...
  if (state == QtLuaEngine::Paused)
    {
      if (theEngine->isPaused())
        {
          bool pause = false;
          emit theApp->anyoneHandlesPause(pause);
          if (! pause)
      ...
      if (!interactionStarted)  
        {
          ...
          theEngine->setPauseOnError(true);
...
/*! \signal anyoneHandlesPause(bool&)
  Applications that can handle pauses in the interpreter
  should  listen to this signal with a direct connection and 
  set the boolean to \a true.  Otherwise \a qlua immediately
  aborts the execution when a pause occurs. 
*/
