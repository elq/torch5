...
static bool
ready_sub(QtLuaEngine::Private *d, QMutexLocker &locker)
{
...
QtLuaEngine::Private::readySlot()
{
  ...
  if (!lockCount && !hopLoop && !hopEvent && !pauseLoop)
    stateChanged = ready_sub(this, locker);
  locker.unlock();
  if (stateChanged)
...
QtLuaEngine::Private::queueSlot()
{
  QMutexLocker locker(&mutex);
  bool stateChanged = false;
  if  (!lockCount && !lockThread && !hopEvent && !pauseLoop)
    stateChanged = ready_sub(this, locker);
  locker.unlock();
  if (stateChanged)
    emit q->stateChanged(QtLuaEngine::Ready);
  ...
  if (!d->pauseLoop && d->rflag && !d->unwindStack)
    {
      ...
      d->condition.wakeOne();
      locker.unlock();
      d->emitStateChanged(QtLuaEngine::Paused);
      if (loop.exec())
        d->unwindStack = true;
      locker.relock();
...
static void
lua_stop_hook(lua_State *L, lua_Debug *)
{
  QtLuaEngine::Private *d = luaQ_private_noerr(L);
  lua_sethook(L, 0, 0, 0);
  ...
  while (level < maxlevel && lua_getstack(L, level++, &ar)) 
    {
      ...
      if (ar.currentline > 0)
        {
          loc = QByteArray(ar.source) + ":" + QByteArray::number(ar.currentline);
...
/*! Stops the lua execution as soon as practicable.
  If property \a pauseOnError is true and if
  flag \a nopause if false, the interpreter
  will transition to the paused state until someone
  calls \a resume(). Otherwise  the interpreter stops executing, 
  unwinds the stack, and returns to ready state. */
...
luaQ_pcall(lua_State *L, int na, int nr, int eh, QObject *obj, bool async)
{
  ...
  if (obj->thread() == QThread::currentThread())
  ...
  else
    {
      ...
      catcher->moveToThread(obj->thread());
...
public:
  virtual ~Receiver();
  Receiver(QtLuaEngine *q) : QObject(q), d(q->d), sender(0), args(0) {}
public slots:
  void universal();
  void disconnect();
...
bool
QtLuaEngine::Receiver::connect(QObject *obj, const char *sig)
{
  Q_ASSERT(sig);
  Q_ASSERT(obj);
  const QMetaObject *m = obj->metaObject();
...
QtLuaEngine::Receiver::universal()
{
  ...
  QThread *mythread = QThread::currentThread();
  QMutexLocker locker(&d->mutex);
  if (d && !d->hopEvent && d->lockCount>0 && d->lockThread==mythread)
    {
      // find closure
      lua_State *L = d->L;
  ...
  while (qs.size() > 0)
    {
      ...
      else if (q.sender && q.receiver && !unwindStack)
        {
  ...
  while (more)
    {
      ...
      locker.relock();
      more = d->processQueuedSignals(locker);
    }
  bool new_rflag = d->rflag;
  d->rflag = saved_rflag;
  locker.unlock();
  if (new_rflag && !saved_rflag)
    emit d->emitStateChanged(QtLuaEngine::Ready);
  if (saved_rflag && !new_rflag)
    emit d->emitStateChanged(QtLuaEngine::Running);
}
...
bool
luaQ_connect(lua_State *L, QObject *obj, const char *sig, int findex)
{
  bool success = false;
  QtLuaEngine::Private *d = luaQ_private(L);
