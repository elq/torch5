...
{
  if (obj == printer || obj == widget && device == (QWidget*)widget)
    resize(0,0,false);
}
...
QtLuaPainter::QtLuaPainter(QtLuaPrinter *printer)
  : QObject(printer), d(new Private(this))
{
  d->printer = printer;
  d->device = printer;
  d->p = new QPainter(printer);
  connect(printer, SIGNAL(closing(QObject*)),
          d, SLOT(destroyed(QObject*)),
          Qt::DirectConnection);
  connect(printer, SIGNAL(destroyed(QObject*)),
          d, SLOT(destroyed(QObject*)),
          Qt::DirectConnection);
  initmatrix();
}
...
{
  if (d->device == (QtLuaPrinter*)d->printer)
    {
      QSizeF sz = d->printer->paperSize();
      if (sz.isValid())
...
QtLuaPainter::gbegin()
{
  d->count += 1;
  // pending showpage
  if (d->eject && d->device == (QtLuaPrinter*)d->printer)
    d->printer->newPage();
  d->eject = false;
}
...
{
  QTransform init;
  QtLuaPrinter *printer = d->printer;
  if (d->device == printer)
    {
