...
public slots:
  ...
  void blockCountChanged(int nb);
  void undoAvailable(bool);
  void redoAvailable(bool);
  void updateUndoRedo();
  void disablePause();
  void anyoneHandlesPause(bool&);
  ...
  if (engine)
    {
      ...
      connect(engine, SIGNAL(errorMessage(QByteArray)),
              this, SLOT(disablePause()));
    }
}


bool 
QLuaSdiMain::Private::luaEval(QByteArray cmd)
{
  QLuaApplication *app = QLuaApplication::instance();
  if (cmd.size() > 0 && cmd[0] == '=')
    cmd = QByteArray("return ") + (cmd.constData() + 1);
...
void
QLuaSdiMain::Private::blockCountChanged(int nb)
{
  const int MARGIN = 4;
...
void 
QLuaSdiMain::Private::disablePause()
{
  canPause = false;
}
...
void 
QLuaSdiMain::Private::anyoneHandlesPause(bool &pause)
{
  if (canPause)
    pause = true;
}
...
QLuaSdiMain::Private::updateLuaActions()
{
  ...
  if (engine && !engine->isReady())
    accepting = false;
  q->stdAction(ActionLuaPause)->setEnabled(!accepting && canPause);
  if (q->hasAction(ActionLuaPause))
    q->stdAction(ActionLuaPause)->setChecked(engine && engine->isPaused());
  if (q->hasAction(ActionLuaStop))
    q->stdAction(ActionLuaStop)->setEnabled(!accepting);
...
QLuaSdiMain::loadSettings()
{
  ...
  else if  (s.contains("editor/font"))
  ...
  // history
  d->historySize = s.value("console/historySize", 1000).toInt();
...
QLuaSdiMain::createMenuBar()
{
  ...
  menu->addSeparator();
  action = addStdActionToMenu(ActionFileSaveAs, menu);
  action->setShortcuts(QKeySequence::Save);
  action->setIcon(QIcon(":/images/filesave.png"));
  ...
  addStdActionToMenu(ActionConsoleClear, menu);
  menu->addSeparator();
  ...
  // lua
  menu = new QMenu(tr("&Lua", "lua|"), this);
  action  = addStdActionToMenu(ActionHistoryUp, menu);
  connect(action, SIGNAL(triggered(bool)), d, SLOT(historyUp()));
  action  = addStdActionToMenu(ActionHistoryDown, menu);
  connect(action, SIGNAL(triggered(bool)), d, SLOT(historyDown()));
  action  = addStdActionToMenu(ActionHistorySearch, menu);
  connect(action, SIGNAL(triggered(bool)), d, SLOT(historySearch()));
  menu->addSeparator();
  action = addStdActionToMenu(ActionLuaEval, menu);
  action->setShortcuts(QList<QKeySequence>() 
                       << QKeySequence(tr("Ctrl+Return","lua|load"))
                       << QKeySequence(tr("Ctrl+Enter","lua|load"))
                       << QKeySequence(tr("F4","lua|load"))
                       << QKeySequence(tr("F5","lua|load")) );
  addStdActionToMenu(ActionLuaPause, menu);
  addStdActionToMenu(ActionLuaStop, menu);
  menubar->addMenu(menu);
  // standard menus
